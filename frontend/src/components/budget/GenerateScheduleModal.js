...
export const GenerateScheduleModal = ({ budowaId, onClose, onSaved }) => {
  ...
  return (
    <Dialog open={true} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="bg-[#1E2A44] border-[#3D5378] text-white max-w-md" data-testid="generate-schedule-modal">
        <DialogHeader>
          <DialogTitle>Generuj harmonogram z budżetu</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div className="text-xs text-[#F1F5F9] bg-[#152033]/60 border border-[#3D5378] rounded p-2">
          ...
          <div>
            <label className="text-xs text-[#CBD5E1]">Data startu pierwszego zadania</label>
            <Input type="date" value={form.start_date} onChange={(e) => setForm({ ...form, start_date: e.target.value })}
              className="bg-[#152033] border-[#3D5378]" data-testid="gen-start-date" />
          </div>
          <div>
            <label className="text-xs text-[#CBD5E1]">Czas trwania pozycji (dni)</label>
            <Input type="number" min="1" max="365" value={form.days_per_position}
              onChange={(e) => setForm({ ...form, days_per_position: e.target.value })}
              className="bg-[#152033] border-[#3D5378]" data-testid="gen-days" />
          </div>
