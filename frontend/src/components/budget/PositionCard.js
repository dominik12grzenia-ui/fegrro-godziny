...
export const PositionCard = ({
  ...
  return (
    <div className="rounded bg-[#1E2A44] border border-[#3D5378] overflow-hidden" data-testid={`position-card-${position.id}`}>
      {/* Naglowek pozycji - klikalny */}
      <div className="flex items-center gap-2 px-2 py-1.5 bg-[#243049] border-b border-[#3D5378]">
        ...
        <div className="hidden sm:flex items-center gap-2 text-[10px] tabular-nums">
          ...
          <span className="px-1.5 py-0.5 rounded bg-[#D4AF37] text-[#152033] font-bold">
            Σ {fmtNum(totals.totalExec)}/{fmtNum(totals.totalPlan)} zł ({totals.totalPct}%)
          </span>
        </div>
        <button onClick={() => onEditPosition(position)} className="text-[#CBD5E1] hover:text-white shrink-0 p-1" data-testid={`position-edit-${position.id}`} title="Edytuj pozycję">
          <Pencil className="h-3.5 w-3.5" />
        </button>
        <button onClick={() => onDeletePosition(position)} className="text-[#CBD5E1] hover:text-[#FCA5A5] shrink-0 p-1" data-testid={`position-del-${position.id}`} title="Usuń pozycję (wraz z wszystkimi kosztami)">
          <Trash2 className="h-3.5 w-3.5" />
        </button>
      </div>
      {!collapsed && (
        <div className="overflow-x-auto">
          <div className="grid grid-cols-3 gap-px bg-[#3D5378] min-w-[900px]">
            {TYPE_ORDER.map((type) => {
              ...
              return (
                <div key={type} className="bg-[#152033] p-2" data-testid={`position-${position.id}-slot-${type}`}>
                    ...
                    <div className="flex items-center gap-0.5 shrink-0">
                      {slot && (
                        <>
                          ...
                          <button onClick={() => onEditLine(slot)} className="text-[#CBD5E1] hover:text-white p-0.5" data-testid={`slot-edit-${slot.id}`} title="Edytuj wartości slotu">
                  ...
                  <div className="text-[10px] grid grid-cols-2 gap-x-2 gap-y-0.5 tabular-nums">
                    ...
                    <span className="text-[#CBD5E1]">Wykonanie:</span>
                    <span className="text-right" style={{ color: cfg.color }}>{fmtNum(td.exec || 0)} zł</span>
                    <span className="text-[#CBD5E1]">Postęp:</span>
                    ...
                    {slot && (slot.kaucja_gir_amount || slot.kaucja_dw_amount) ? (
                      <>
                        <span className="text-[#CBD5E1]">Kaucje:</span>
                        <span className="text-[#CBD5E1] text-right">{fmtNum((slot.kaucja_gir_amount || 0) + (slot.kaucja_dw_amount || 0))} zł</span>
                      </>
                    ) : null}
                  </div>
                  {/* Skladowe */}
                  {children.length > 0 && (
                    <div className="mt-2 pt-1.5 border-t border-[#3D5378]/50 space-y-0.5">
                      <div className="text-[9px] text-[#94A3B8] uppercase tracking-wide mb-0.5">Składowe ({children.length})</div>
                      {children.map((c) => (
                        <div key={c.id} className="flex items-center justify-between gap-1 text-[10px] hover:bg-[#243049]/60 px-1 py-0.5 rounded">
                          <div className="flex items-center gap-1 min-w-0">
                            <span className="text-[#D4AF37] shrink-0">↳</span>
                            <span className="text-[#F1F5F9] truncate" title={c.name}>{c.name}</span>
                          </div>
                          <div className="flex items-center gap-1 shrink-0">
                            <span className="text-white tabular-nums">{fmtNum(c.plan_netto_computed || 0)}</span>
                            <button onClick={() => onEditLine(c)} className="text-[#CBD5E1] hover:text-white p-0.5" data-testid={`child-edit-${c.id}`} title="Edytuj składową">
                              <Pencil className="h-2.5 w-2.5" />
                            </button>
                            <button onClick={() => onDeleteLine(c.id)} className="text-[#CBD5E1] hover:text-[#FCA5A5] p-0.5" data-testid={`child-del-${c.id}`} title="Usuń składową">
