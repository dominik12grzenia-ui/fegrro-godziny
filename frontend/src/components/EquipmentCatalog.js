...
export const EquipmentCatalog = ({ category = 'electronics' }) => {
  ...
  if (loading) {
    return (
      <div className="py-6 text-center text-[#CBD5E1] text-sm">Ładowanie katalogu...</div>
    );
  }
  ...
  return (
    <>
      ...
      {catalogOpen && (
        ...
        >
          <div className="bg-[#243049] border-2 border-[#4F6343] rounded-lg shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col">
            <div className="px-5 py-4 border-b border-[#3D5378] flex items-center justify-between gap-3 flex-wrap">
              ...
              <Input
                ...
                className="bg-[#1E2A44] border-[#3D5378] text-white w-full sm:w-auto sm:min-w-[240px]"
            ...
            <div className="p-5 overflow-y-auto">
              {visibleCatalog.length === 0 ? (
                <div className="text-center py-6 text-sm text-[#CBD5E1]">
                  {catalog.length === 0
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {visibleCatalog.map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center gap-3 p-2 bg-[#1E2A44] rounded border border-[#3D5378]"
                      data-testid={`catalog-item-${item.id}`}
                    >
                      {item.photo ? (
                        <img src={item.photo} alt={item.name} className="h-12 w-12 rounded object-cover border border-[#3D5378] shrink-0" />
                      ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-[#F1F5F9] text-sm font-semibold truncate">{item.name}</div>
                        {item.brand && <div className="text-xs text-[#CBD5E1] truncate">{item.brand}</div>}
                        <div className="text-xs text-[#CBD5E1] mt-0.5">
                          ...
                          <span className="text-[#94A3B8]"> / {item.total_quantity}</span>
      ...
      {(activeOrders.length > 0 || recentOrders.length > 0) && (
        <Card className="bg-[#243049] border-[#3D5378] mb-4" data-testid="my-equipment-orders">
          <CardHeader>
            ...
            <div className="space-y-2">
                  ...
                  >
                    ...
                    {o.status === 'pending' && (
                      <Button
                        ...
                        className="text-[#CBD5E1] hover:text-[#FCA5A5] hover:bg-[#9B2C2C]/20 shrink-0 h-8 w-8 p-0"
              ...
              {recentOrders.map((o) => {
                ...
                return (
                  <div
                    key={o.id}
                    className="flex items-center gap-3 p-2 bg-[#152033] rounded border border-[#3D5378]/50 opacity-70"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="text-[#CBD5E1] text-sm truncate">
                        {o.equipment_name}
                        {o.variant && <span> - {o.variant}</span>}
                        <span className="ml-2 text-[#94A3B8]">x{o.quantity_issued || o.quantity_requested}</span>
                      </div>
                    </div>
                    <span className={`px-2 py-0.5 rounded text-xs border ${badge.cls}`}>{badge.label}</span>
                  </div>
      ...
      {modalItem && (
        ...
        >
          <div className="bg-[#243049] border-2 border-[#4F6343] rounded-lg shadow-2xl max-w-md w-full">
            <div className="px-5 py-4 border-b border-[#3D5378] flex items-center justify-between">
              ...
              <button onClick={() => setModalItem(null)} className="text-[#CBD5E1] hover:text-white" data-testid="order-close-btn">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="p-5 space-y-3">
              <div className="flex items-center gap-3 p-2 bg-[#1E2A44] rounded">
                {modalItem.photo ? (
                  <img src={modalItem.photo} alt={modalItem.name} className="h-14 w-14 rounded object-cover" />
                ) : (
                  <div className="h-14 w-14 rounded bg-[#3D5378] flex items-center justify-center">
                    <Package className="h-6 w-6 text-[#CBD5E1]" />
                  </div>
                )}
                <div>
                  <div className="text-[#F1F5F9] font-semibold">{modalItem.name}</div>
                  {modalItem.brand && <div className="text-xs text-[#CBD5E1]">{modalItem.brand}</div>}
                  <div className="text-xs text-[#CBD5E1]">Dostępne: {modalItem.available_quantity} szt.</div>
                </div>
              </div>
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded border text-sm font-bold transition-colors ${
                          orderVariant === v
                            ...
                            : 'bg-[#1E2A44] border-[#3D5378] text-[#F1F5F9] hover:border-[#4F6343]/50'
                        }`}
              ...
              <div>
                <label className="text-sm text-[#F1F5F9] block mb-1">Ilość</label>
                <Input
                  type="number"
                  min="1"
                  value={orderQty}
              ...
              <div>
                <label className="text-sm text-[#F1F5F9] block mb-1">Uwagi (opcjonalnie)</label>
                <textarea
                  value={orderNotes}
            ...
            <div className="px-5 py-4 border-t border-[#3D5378] flex justify-end gap-2">
              <Button onClick={() => setModalItem(null)} variant="ghost" className="text-[#CBD5E1]" data-testid="order-cancel-btn">
                Anuluj
              </Button>
              <ActionButton
