          ...
          <CardContent className="space-y-3">
                ...
                {penaltyList.map(pen => (
                  <div key={pen.id} className="p-3 bg-[#131C2F] rounded-lg border border-[#2A3B59]">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[#64748B] text-[10px]">{pen.created_at ? new Date(pen.created_at).toLocaleString('pl-PL') : ''}</span>
                    </div>
                    {pen.description && <p className="text-[#94A3B8] text-xs mb-2">{pen.description}</p>}
                    {pen.image_data && (
            ...
            <div className="p-3 bg-[#0B1120] rounded-lg border border-[#9B2C2C]/20">
              ...
              <Input value={newPenaltyDesc} onChange={e => setNewPenaltyDesc(e.target.value)}
                placeholder="Opis kary" className="bg-[#131C2F] text-white border-[#2A3B59] text-sm mb-2"
              ...
              <div className="flex gap-2 items-center mb-2">
                ...
                {newPenaltyImage && <img src={newPenaltyImage} alt="Preview" className="h-10 w-10 rounded object-cover border border-[#2A3B59]" />}
              </div>
