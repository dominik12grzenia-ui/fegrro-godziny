            ...
            {historyData.transfers.length === 0 ? (
              ...
              <div className="space-y-1" data-testid="my-history-transfers">
                {historyData.transfers.map((tr) => {
                  ...
                  return (
                    <div key={tr.id} className="text-xs p-2 bg-[#131C2F] rounded border border-[#2A3B59]">
                      ...
                      <span className="text-[#CBD5E1]">{tr.equipment_name} x {tr.quantity}</span>{' '}
                      <span className="text-[#94A3B8]">·</span>{' '}
                      ...
                      <span className="text-[#64748B]">· {new Date(tr.created_at).toLocaleString('pl-PL')}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="space-y-1">
                {historyData.events.map((e) => (
                  <div key={e.id} className="text-xs p-2 bg-[#131C2F] rounded border border-[#2A3B59]">
                    ...
                    <span className="text-[#CBD5E1]">{e.details?.equipment_name || ''} x {e.details?.quantity || '?'}</span>
                    {e.details?.description && <span className="text-[#94A3B8]"> · "{e.details.description}"</span>}{' '}
                    <span className="text-[#64748B]">· {new Date(e.created_at).toLocaleString('pl-PL')}</span>
                  </div>
                ))}
              </div>
            </div>
